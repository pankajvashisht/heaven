...
class adminController {
  ...
  async allUser(req) {
    ...
    let query =
      ...
      ' , ' +
      limit;
    const total =  await DB.first(`select count(*) as total from users ${conditions}`);
    const result={
  ...
  async seeds(req) {
    ...
    let query =
      ...
      ' , ' +
      limit;
    const total =  await DB.first(`select count(*) as total from seeds join users on (users.id = seeds.user_id) ${conditions}`);
    const result={
      pagination:{page:offset,total:total[0].total,limit:limit},
      result:app.addUrl(await DB.first(query), 'profile')
    };  
  ...
  async tithe(req){
    ...
    let query =
      ...
      ' , ' +
      limit;
    const total =  await DB.first(`select count(*) as total from tithe join users on (users.id = tithe.user_id) ${conditions}`);
    const result={
      pagination:{page:offset,total:total[0].total,limit:limit},
      result:app.addUrl(await DB.first(query), 'profile')
    };  
  ...
  async addUser(req) {
    ...
    const result = await DB.first(`select * from users where email = '${body.email}' or phone = '${body.phone}'`);
    if (result.length) {
  ...
  async dashboard() {
    ...
    totals.tithe = await DB.first(
      'select count(*) as total from tithe',
    );
    totals.users = await DB.first('select count(*) as total from users');
    totals.seeds = await DB.first(
      'select count(id) as total from 	seeds',
    );
  ...
  async transaction(req) {
    ...
    let query =
      ...
      ' , ' +
      limit;
    const total =  await DB.first(`select count(*) as total from transactions join users on (users.id = transactions.user_id) ${conditions}`);
    const result={
      pagination:{page:offset,total:total[0].total,limit:limit},
      result:app.addUrl(await DB.first(query), 'profile')
    };  
