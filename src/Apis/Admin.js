...
export const UserList = ({ page, query, limit }) => {
  return axios.get(`${apis}/users/${page}/${limit}?token=${login_datails.token}&q=${query}`, {
    
  });
};
export const Seeds = ({ page, query, limit }) => {
  return axios.get(`${apis}/seeds/${page}/${limit}?token=${login_datails.token}&q=${query}`, {
    
  });
};
export const Tithe = ({ page, query, limit }) => {
  return axios.get(`${apis}/tithe/${page}/${limit}?token=${login_datails.token}&q=${query}`, {
    
  });
};
export const Transaction = ({ page, query, limit }) => {
  return axios.get(`${apis}/transaction/${page}/${limit}?token=${login_datails.token}&q=${query}`, {
